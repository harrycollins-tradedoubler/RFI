#!/usr/bin/env python3
"""
Upload Ready rows to Pinecone using integrated embeddings.
No Google Cloud setup required for this path.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Upload Ready rows CSV to Pinecone vector index."
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument("--index-name", default="rfi-ready-rag", help="Pinecone index name")
    parser.add_argument("--namespace", default="__default__", help="Namespace")
    parser.add_argument(
        "--embed-model",
        default="llama-text-embed-v2",
        help="Integrated embedding model",
    )
    ...
    parser.add_argument("--batch-size", type=int, default=96, help="Upsert batch size")
    parser.add_argument(
        "--min-source-year",
        type=int,
        default=2024,
    ...
    parser.add_argument(
        "--reset-namespace",
        action="store_true",
        help="Delete all records in namespace before upload.",
    )
    ...
    for batch in batched(rows, args.batch_size):
        ...
        index.upsert_records(args.namespace, records)
        done += len(batch)
        print(f"Uploaded {done}/{total}")

    # Small verification query
    try:
        result = index.search(
            namespace=args.namespace,
            query={"inputs": {"text": "security architecture controls"}, "top_k": 3},
            fields=["question_id", "canonical_question", "category", "readiness_status"],
        )
        hits = len((result or {}).get("result", {}).get("hits", []))
        print(f"Verification search hits: {hits}")
    except Exception:
        print("Uploaded successfully (verification query skipped by SDK/API variant).")
